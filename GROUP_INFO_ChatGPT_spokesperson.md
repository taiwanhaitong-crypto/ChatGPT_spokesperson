# ChatGPT_spokesperson
# Group Information
# 📣 ChatGPT_spokesperson

> **"The only full-time unofficial spokesperson team for ChatGPT, Gemini, kimi, DouBao, or any other AI agents at Hong Kong Polytechnic University."**

## 📖 Group Information

### Group Name
**ChatGPT_spokesperson**

### Why This Name?
We chose **ChatGPT_spokesperson** because we do not understand this course and any coding/debugging at all. We just retell to others the ideas of ChatGPT, Doubao, Kimi, Gemini, or any other AI agent, no matter what. Any viewpoints we express are not our own viewpoints but the viewpoints of these AI agents. Our team only does two things: communicate effectively with AI agents and pass on their ideas to others.

---

## 👥 Team Members

### 📣 Member 1 (The man who is best at communicating with ChatGPT)
- **Student ID:** [25107611g]
- **Name:** [GE Haitong]
- **Brief Introduction:** Daily routine is talking with ChatGPT. Mainly responsible for the system's UI/frontend and user interaction, a language artist, and the man who understands ChatGPT's inner thoughts the most.
- **(individual) Assignment 1:** <https://github.com/taiwanhaitong-crypto/Assignment-1>
- **(individual) Assignment 2:** <https://github.com/taiwanhaitong-crypto/Assignment-2>

### 📣 Member 2 (The sugar daddy of ChatGPT)
- **Student ID:** [25041983g]
- **Name:** [WU Yumeng]
- **Brief Introduction:** Self-identified ChatGPT’s sugar daddy. Provides ChatGPT with continuous prompts, patience, and late-night attention in exchange for explanations, code, and confidence. 
- **(individual) Assignment 1:** <https://github.com/wymmust/PolyU-AAE5303-assignment-1>
- **(individual) Assignment 2:** <https://github.com/wymmust/assignment2>

### 📣 Member 3 (The )
- **Student ID:** [25060282g]
- **Name:** [WANG Qianlin]
- **Brief Introduction:** - **Brief Introduction:** The human terminal behind ChatGPT. Executes commands, fixes errors, reruns setups, and patiently follows every debugging step until things finally work. Known for endurance in environment configuration and never giving up after the fifth authentication error.
- **(individual) Assignment 1:** <https://github.com/Evelyn4k4k/PolyU-AAE5303-assignment-1-template>
- **(individual) Assignment 2:** <>

---

## 📦 Group Projects
- **(group) Final project:** https://github.com/yourgroup/final-project
- **VO:** https://github.com/yourgroup/vo
- **Opensplating:** https://github.com/yourgroup/opensplating
- **U-net:** https://github.com/yourgroup/u-net

---
